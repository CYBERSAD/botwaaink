...
const starts = async (cyber = new WAConnection()) => {
    ...
    gam = img
    jadinya = await cyber.prepareMessage(from, gam, MessageType.image)
    buttonMessagesI = {
    ...
    cyber.sendMessage(from, buttonMessagesI, MessageType.buttonsMessage)
  }

    fs.existsSync('./cyber.json') && cyber.loadAuthInfo('./cyber.json')
    cyber.on('connecting', () => {
        start('2', 'Menghubungkan...')
    })
    cyber.on('open', () => {
        success('2', 'Terhubung Jangan Lupa Subscribe Mr Cyber Sad')
    })
    await cyber.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./cyber.json', JSON.stringify(cyber.base64EncodedAuthInfo(), null, '\t'))

    cyber.on('chat-update', async (message) => {
        require('./cyber.js')(cyber, message, _welkom)
    })
cyber.on("group-participants-update", async (anu) => {

    const isWelkom = _welkom.includes(anu.jid)
    try {
      groupMet = await cyber.groupMetadata(anu.jid)
      ...
      console.log(anu)
      try {
        pp_user = await cyber.getProfilePicture(mem)
      } catch (e) {
        pp_user = "https://telegra.ph/file/c9dfa715c26518201f478.jpg"
      }
      try {
        pp_grup = await cyber.getProfilePicture(anu.jid)
      } catch (e) {
        pp_grup =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60"
      }
      if (anu.action == "add" && mem.includes(cyber.user.jid)) {
        cyber.sendMessage(anu.jid, "Halo!.. saya ZeroBot saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk meulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(cyber.user.jid)) {
        mdata = await cyber.groupMetadata(anu.jid)
        memeg = mdata.participants.length
        num = anu.participants[0]
        let v = cyber.contacts[num] || { notify: num.replace(/@.+/, "") }
        anu_user = v.vname || v.notify || num.split("@")[0]
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
        wel = `Halo @${anu_user} \nWelcome In ${mdata.subject} \nJangan Lupa Follow Sosial Media Ownerku\n• Youtube : https://youtube.com/Mr Cyber Sad\n• Instagram : https://instagram.com/bocah editod\n• Tiktok : https://tiktok.com/@mr0red\n• Github : https://github.com/Zero-YT7`
        ...
        sendButImage(mdata.id, wel, "©Created : Mr Cyber Sad", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(cyber.user.jid)) {
        mdata = await cyber.groupMetadata(anu.jid)
        num = anu.participants[0]
        let w = cyber.contacts[num] || { notify: num.replace(/@.+/, "") }
        ...
        sendButImage(mdata.id, out, "©Created : Mr Cyber Sad", buff, but)
      }
      if (anu.action == "promote") {
        const mdata = await cyber.groupMetadata(anu.jid)
        anu_user = cyber.contacts[mem]
        num = anu.participants[0]
        try {
          ppimg = await cyber.getProfilePicture(
        ...
        cyber.sendMessage(mdata.id, teks, MessageType.text)
      }
